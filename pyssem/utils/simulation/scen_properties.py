...
def parallel_lambdify(equations_flattened, all_symbolic_vars):
    ...
    with get_reusable_executor(max_workers=num_cores) as executor:
        futures = [executor.submit(
            lambdify_equation, all_symbolic_vars, eq) for all_symbolic_vars, eq in args]
...
class ScenarioProperties:
    def __init__(self, start_date: datetime, simulation_duration: int, steps: int, min_altitude: float,
        ...
        if self.density_model == "static_exp_dens_func":
            ...
            print(
                "Warning: Unable to parse density model, setting to static exponential density model")
            self.density_model = static_exp_dens_func
        ...
        R0 = np.linspace(self.min_altitude, self.max_altitude,
                         self.n_shells + 1)  # Altitude of the shells [km]
        # semi-major-axis midpoints in meters:
        R0_alt_km = np.linspace(
            self.min_altitude, self.max_altitude, self.n_shells + 1)
        ...
        # Convert to meters and the radius of the earth
        R0 = (self.re + R0) * 1000
        self.V = 4 / 3 * pi * np.diff(R0**3)  # volume of the shells [m^3]
        if self.v_imp is not None:
            # impact velocity [km/s] Shell-wise
            self.v_imp2 = self.v_imp * np.ones_like(self.V)
        else:
            # Calculate v_imp for each orbital shell using the vis viva equation
            # impact velocity [km/s] Shell-wise
            self.v_imp2 = np.sqrt(2 * self.mu / (self.HMid * 1000)) / 1000
        ...
        # Integration options # these are likely to change
        self.options = {'reltol': 1.e-4, 'abstol': 1.e-4}
        ...
        self.species = []
        self.species_types = []
        # dict with S, D, N, Su, B arrays or whatever species types exist}
        self.species_cells = {}
        self.species_names = []
        self.debris_names = []
    ...
    def calculate_scen_times_dates(self):
        ...
        scen_times_dates = np.empty_like(
            self.scen_times, dtype='datetime64[M]')
    ...
    def future_launch_model(self, FLM_steps):
        ...
        if elliptical:
                ...
                for species in species_group:
                    if species.sym_name not in FLM_steps.columns:
                        if elliptical:
                            ...
                            species.lambda_funs = [
                                0 for _ in range(n_shells * self.n_ecc_bins)]
                        else:
                    ...
                    if elliptical:
                        ...
                        temp_df = FLM_steps.loc[:, [
                            'alt_bin', 'ecc_bin', 'epoch_start_date', species.sym_name]].copy()
                        temp_df[species.sym_name] = pd.to_numeric(
                            temp_df[species.sym_name], errors='coerce')
                        temp_df[species.sym_name] = temp_df[species.sym_name].replace(
                        ...
                        if not np.issubdtype(temp_df['epoch_start_date'].dtype, np.datetime64):
                            temp_df['epoch_start_date'] = pd.to_datetime(
                                temp_df['epoch_start_date'], errors='coerce', utc=True)

                        # Flat list of length n_shells * n_ecc_bins
                        lambda_funs = [0 for _ in range(
                            n_shells * self.n_ecc_bins)]

                        grouped = temp_df.groupby(
                            ['alt_bin', 'ecc_bin'], sort=True)
                        for (shell, ecc_bin), group in grouped:
                            ...
                            flat_index = int(shell) * \
                                self.n_ecc_bins + int(ecc_bin)
                            lambda_funs[flat_index] = y if (
                                y.size > 0 and np.any(y != 0.0)) else 0
                        ...
                        if species.sym_name == 'B':
                            nan_cells = sum(int(np.isnan(entry).any(
                            )) for entry in lambda_funs if isinstance(entry, np.ndarray))
                            print(
                                f"Species: {species.sym_name} Total Count: {total_count} (cells with NaNs: {nan_cells})")
                        else:
                            print(
                                f"Species: {species.sym_name} Total Count: {total_count}")
                    else:
                        temp_df = FLM_steps.loc[:, [
                            'alt_bin', 'epoch_start_date', species.sym_name]]
                        species_FLM = temp_df.pivot(
                            index='alt_bin', columns='epoch_start_date', values=species.sym_name)

                        lambda_funs = []
                        if species.launch_altitude is not None:
                            closest_shell = np.argmin(
                                np.abs(self.HMid - species.launch_altitude))
                        else:
                            closest_shell = None
            ...
            if len(np.unique(np.round(np.diff(scen_times), 5))) == 1:
                ...
                raise ValueError(
                    "FLM to Launch Function is not set up for variable time step runs.")
        ...
        try:
            for indicator in self.indicator_variables:
                if indicator == "orbital_volume":
                    ...
                    self.indicator_variables_list.append(
                        make_all_col_indicators(self))

        except Exception as e:
            print(
                f"An error occurred creating the indicator variables: {str(e)}")
    ...
    def initial_pop_and_launch(self, baseline=False, launch_file=None):
        ...
        launch_file_path = os.path.join(
            'pyssem', 'utils', 'launch', 'data', f'ref_scen_{launch_file}.csv')
        ...
        if not os.path.exists(launch_file_path):
            raise FileNotFoundError(
                f"Launch file {launch_file_path} does not exist. Please provide a valid launch file.")
    ...
    def build_model(self):
        ...
        species_list = [species for group in self.species.values()
        ...
        for i, species in enumerate(species_list):
            ...
            lambda_expr = species.launch_func(
                self.scen_times, self.HMid, species, self)
            ...
            # Drag
            [upper_term, current_term] = species.drag_func(
                t, self.HMid, species, self)
            try:
        ...
        if not self.time_dep_density:
            ...
            drag_upper_with_density = self.drag_term_upper.multiply_elementwise(
                upper_rho)
            drag_cur_with_density = self.drag_term_cur.multiply_elementwise(
                current_rho)
        ...
        if hasattr(self, 'integrated_indicator_var_list'):
            ...
            for ind_var in integrated_indicator_var_list:
                ...
                ind_var.indicator_idxs = list(
                    range(start_indicator_idxs, end_indicator_idxs + 1))

                self.xdot_eqs = sp.Matrix.vstack(
                    self.xdot_eqs, sp.Matrix(ind_var.eqs))

            if not self.sym_lambda:
                indicator_pad = [lambda x, t: 0] * \
                    self.num_integrated_indicator_vars
        ...
        collisions_flattened = [self.full_coll[i, j] for j in range(
            self.full_coll.cols) for i in range(self.full_coll.rows)]
    ...
    def build_model_elliptical(self):
        ...
        self.equations, self.full_lambda_flattened = self.lambdify_equations(
        ), self.lambdify_launch_elliptical()
    ...
    def build_sym_model(self):
        ...
        # self.equations = self.full_Cdot_PMD + self.full_coll
        self.equations = self.full_Cdot_PMD + self.full_coll + \
            self.full_lambda + self.full_control
    ...
    def lambdify_equations(self):
        ...
        equations_flattened = [self.equations[i, j] for j in range(
            self.equations.cols) for i in range(self.equations.rows)]

        # Convert the equations to lambda functions
        if self.parallel_processing:
            equations = parallel_lambdify(
                equations_flattened, self.all_symbolic_vars)
        else:
            equations = [sp.lambdify(self.all_symbolic_vars, eq, 'numpy')
    ...
    def sma_ecc_mat_to_altitude_mat(self, population_matrix_sma_ecc):
        ...
        try:
            ecc_mid = np.asarray(self.binE_ecc_mid_point,
                                 dtype=float)   # (n_ecc,)
            sma_mid = np.asarray(
                self.sma_HMid_km, dtype=float)          # (n_sma,)
        except AttributeError:
            ecc_mid = np.asarray(
                self.scenario_properties.binE_ecc_mid_point, dtype=float)
            sma_mid = np.asarray(
                self.scenario_properties.sma_HMid_km, dtype=float)

        # --- Perigee filter: keep only a(1-e) > R_earth + 150 km ---
        R_earth_km = getattr(
            self, "R_earth_km",
            getattr(getattr(self, "scenario_properties", self),
                    "R_earth_km", 6378.137)
        )
        ...
        # both (n_sma, n_ecc)
        A_km, E = np.meshgrid(sma_mid, ecc_mid, indexing="ij")
        rp_km = A_km * (1.0 - E)
        keep_mask = rp_km > (
            R_earth_km + perigee_altitude_threshold_km)  # True => keep

        # Zero out decaying cells across all species
        pop_filtered = population_matrix_sma_ecc * \
            keep_mask[:, None, :]  # (n_sma, n_species, n_ecc)

        # --- Map to altitude via time-in-shell weights ---
        # time_in_shell: (alt, ecc, sma); pop_filtered: (sma, species, ecc)
        # Result: (alt, species)
        effective_altitude = np.einsum(
            "aes, spe -> ap", self.time_in_shell, pop_filtered, optimize=True)
    ...
    def population_rhs(self, t, x_flat, launch_funcs, n_sma_bins, n_species, n_ecc_bins, n_alt_shells,
        ...
        #############################
        # shape: (sma_shells, species, ecc)
        x_matrix = x_flat.reshape((n_sma_bins, n_species, n_ecc_bins))
        # shape: (alt_shells, sma_shells, ecc)
        time_in_shell = self.time_in_shell
        ...
        normalised_species_distribution_in_sma_e_space = np.zeros(
            (n_alt_shells, n_species, n_sma_bins, n_ecc_bins))
                ...
                for alt_shell in range(n_alt_shells):
                    ...
                    for sma in range(n_sma_bins):
                        for ecc in range(n_ecc_bins):
                            ...
                            normalised_species_distribution_in_sma_e_space[alt_shell,
                                                                           species, sma, ecc] = n_effective_a_e

                    normalised_species_distribution_in_sma_e_space[alt_shell, species, :, :] = (
                        normalised_species_distribution_in_sma_e_space[alt_shell, species, :, :] / n_effective)
                    # convert any nans to 0
                    normalised_species_distribution_in_sma_e_space[alt_shell, species, :, :] = np.nan_to_num(
                        normalised_species_distribution_in_sma_e_space[alt_shell, species, :, :])
                    self.effective_altitude_matrix[alt_shell,
                                                   species] = n_effective
        except Exception as e:
            print(f"Error in calculating effective altitude matrix: {e}")
            raise ValueError(
                "The population matrix is not defined correctly. Please check your population matrix.")

        total_dNdt_alt = np.zeros((n_alt_shells, n_species))
        total_dNdt_sma_ecc_sources = np.zeros(
            (n_sma_bins, n_species, n_ecc_bins))
        ...
        for term in self.collision_terms:
            ...
            for shell in range(n_alt_shells):
                for species in range(n_species):
                    ...
                    # this should be to equal to on
                    sma_ecc_distribution = term.spread_distribution[shell,
                                                                    mass_bin, :, :]
                    # get the column of the debris species
                    species_frag = total_dNdt_alt[shell, species]
                    ...
                    total_dNdt_sma_ecc_sources[:, species,
        ...
        for term in self.collision_terms:  # for each species pair
            dNdt_term = term.lambdified_sinks(
                *x_flat_ordered)  # n_shells x n_species

            # for each species essentially find where the fragments came from (using effective pop)
            for species in range(n_species):
                for shell in range(n_alt_shells):
                    ...
                    dNdt_sink_sma_ecc[:, species, :] = dNdt_sink_sma_ecc[:,
                                                                         species, :] + frag_sink_sma_ecc
                    # if frag_sink_sma_ecc has any nans stop
                    if np.isnan(dNdt_sink_sma_ecc).any():
                        raise ValueError(
                            f"NaN found in dNdt_sink_sma_ecc for species {species} at shell {shell}. Check your collision equations.")
        ...
        for species in range(n_species):
            ...
            if all_species_list[species].active == True:
                for sma in range(n_sma_bins):
                    for ecc in range(n_ecc_bins):
                        pmd[sma, species, ecc] -= (
                            1 / all_species_list[species].deltat) * x_matrix[sma, species, ecc]
    ...
    def run_model(self):
        ...
        if self.time_dep_density:
            ...
            drag_upper_flattened = [self.drag_term_upper[i, j] for j in range(
                self.drag_term_upper.cols) for i in range(self.drag_term_upper.rows)]
            drag_current_flattened = [self.drag_term_cur[i, j] for j in range(
                self.drag_term_cur.cols) for i in range(self.drag_term_cur.rows)]

            self.drag_upper_lamd = [sp.lambdify(
                self.all_symbolic_vars, eq, 'numpy') for eq in drag_upper_flattened]
            self.drag_cur_lamd = [sp.lambdify(
                self.all_symbolic_vars, eq, 'numpy') for eq in drag_current_flattened]

            # Set up time varying density
            self.density_data = preload_density_data(os.path.join(
                # HOW TO CHANGE THE SSP
                'pyssem', 'pyssem', 'utils', 'drag', 'dens_SSP1-26_2000-2100.json'))
            ...
            self.date_mapping = precompute_date_mapping(pd.to_datetime(
                self.start_date), pd.to_datetime(self.end_date), self.steps)

            # This will change when jb2008 is updated
            available_altitudes = list(
                map(int, list(self.density_data['2020-03'].keys())))
            available_altitudes.sort()

            self.nearest_altitude_mapping = precompute_nearest_altitudes(
                available_altitudes)
            ...
            self.progress_bar = tqdm(
                total=self.scen_times[-1] - self.scen_times[0], desc="Integrating Equations", unit="year")
                ...
                for rate_array in self.full_lambda_flattened:
                    try:
                        if rate_array is not None:
                            ...
                            clean_rate_array[np.isnan(clean_rate_array)] = 0
                            # Replace any infinity values (positive or negative) with 0.
                            clean_rate_array[np.isinf(clean_rate_array)] = 0
            ...
            for i in self.indicator_variables_list:
                ...
                for indicator_var in i:
                    try:
                        ...
                        indicator_fun = sp.lambdify(
                            self.all_symbolic_vars, simplified_eqs, 'numpy')
    ...
    def make_rate_interpolator(self, times, rates, method="pchip", extrap="hold", smooth=None, values='counts'):
        ...
        # choose interpolator
        if method == "pchip":
            base = PchipInterpolator(
                t, y, extrapolate=(extrap == "extrapolate"))
        ...
        if method == "bspline":
            ...
            if values == "counts":
                if len(t) != len(y) + 1:
                    raise ValueError(
                        "For method='bspline' with values='counts', 'times' must be interval EDGES (len = len(counts)+1).")
                dt = np.diff(t)
        ...
        if values == "counts":
            ...
            if n < 2:
                raise ValueError(
                    "values='counts' requires at least 2 time edges.")
            ...
            def _f_counts(tt, t_edges=t_edges, r=r):
                tt = np.asarray(tt, float)
                k = np.searchsorted(
                    t_edges, tt, side='right') - 1  # interval index
                k = np.clip(k, 0, len(r)-1)
                out = r[k]
                if extrap == "zero":
                    out = np.where((tt < t_edges[0]) | (
                        tt >= t_edges[-1]), 0.0, out)
                elif extrap == "hold":
                    left = (tt < t_edges[0])
                    right = (tt >= t_edges[-1])
    ...
    def run_model_elliptical(self):
        ...
        if self.time_dep_density:
            ...
            # Set up time varying density
            self.density_data = preload_density_data(os.path.join(
                'pyssem', 'utils', 'drag', 'dens_highvar_2000_dens_highvar_2000_lookup.json'))
            ...
            for term in self.collision_terms:
                ...
                totals = term.fragment_spread_totals.sum(
                    axis=(2, 3), keepdims=True)  # shape: (shell, mass, 1, 1)
                ...
                if np.sum(spread_distribution) == 0 and np.sum(totals) != 0:
                    print(
                        f"Warning: No fragments produced for term {term.name}. Check your collision parameters.")
            ...
            for term in self.collision_terms:
                term.lambdified_sources = sp.lambdify(
                    flat_vars, term.eqs_sources, modules="numpy")
                term.lambdified_sinks = sp.lambdify(
                    flat_vars, term.eqs_sinks, modules="numpy")

            # Map species index to mass bin index, only for debris
            species_to_mass_bin = {
                i: j for j, (i, name) in enumerate(
                    [(i, name) for i, name in enumerate(
                        self.species_names) if name.startswith("N")]
                )
            }
                ...
                for sma in range(self.n_sma_bins):
                    for species in range(n_species):
                        for ecc in range(self.n_ecc_bins):
                            rate_array = self.full_lambda_flattened[sma,
                                                                    species, ecc]
                            ...
                            try:
                                ...
                                # Case 2: mixed list of array + zeros
                                    ...
                                    array_found = next(
                                        (np.asarray(r).astype(float)
                                         for r in rate_array if isinstance(r, np.ndarray)),
                                        None
                                    )
                                ...
                                # Clean
                                flattened_array[np.isnan(
                                    flattened_array)] = 0.0
                                flattened_array[np.isinf(
                                    flattened_array)] = 0.0
                                ...
                                spline_func, _, _ = self._zero_padded_spline(
                                    self.scen_times, flattened_array, bc_type="natural")
                                launch_rate_functions[sma,
                                                      species, ecc] = spline_func
            ...
            self.full_Cdot_PMD = [sp.lambdify(
                flat_vars, eq, 'numpy') for eq in self.full_Cdot_PMD]
            ...
            for bstar in bstar_vals:
                ...
                param = {
                    ...
                    # 2.2000e-08, # 2.2 * ((2.687936011/1e3)**2/ 1783),  # bstar = cd * ((radius/1e3)**2/ mass) 0.5, 148
                    'Bstar': bstar,
                    'j2': 1082.63e-6
                }
            ...
            all_species_list = [
                species for category in self.species.values() for species in category]
        ...
        if hasattr(self, 'indicator_variables_list'):
            ...
            x_matrix = self.output.y.reshape(
                self.n_sma_bins, n_species, self.n_ecc_bins, n_time)

            # --- Project to altitude shells over time ---
            # (alt/shell, species, time)
            n_eff = np.zeros((self.n_shells, n_species, n_time))
            for s in range(n_species):
                for t_idx, _ in enumerate(self.output.t):
                    ...
                    cube = np.zeros(
                        (self.n_sma_bins, n_species, self.n_ecc_bins))
                    cube[:, s, :] = snap
                    alt_proj = self.sma_ecc_mat_to_altitude_mat(
                        cube)  # (alt, species)
                    n_eff[:, s, t_idx] = alt_proj[:, s]

            # shape: (n_alt_shells, n_species, n_time)
            self.output.y_alt = n_eff
            ...
            if n_shells != self.n_shells or n_species_chk != self.species_length:
                raise ValueError(
                    "Shape mismatch: y_alt dims do not match self.n_shells/self.species_length.")
            if n_time_chk != len(self.output.t):
                raise ValueError(
                    "Time axis mismatch: len(self.output.t) != y_alt.shape[2].")
            ...
            for group in self.indicator_variables_list:
                for indicator_var in group:
                    try:
                        ...
                        print(
                            f"Cannot make indicator for {getattr(indicator_var, 'name', '<unnamed>')}")
                        print(e)
    ...
    def _build_symbol_order_map(self):
        ...
        for sym in self.all_symbolic_vars:
            ...
            if sep == '' or not shell_str.isdigit():
                raise ValueError(
                    f"Symbol '{name}' must end with '_<shell_index>' (1-based).")
            ...
            if base not in species_to_idx:
                ...
                raise KeyError(
                    f"Symbol base '{base}' not found in species_names {self.species_names}.")

            species_idx = species_to_idx[base]
            if not (0 <= shell_idx < self.n_shells):
                raise IndexError(
                    f"Shell index {shell_idx} out of range for symbol '{name}' with n_shells={self.n_shells}.")
        ...
        if len(order_map) != expected:
            raise ValueError(
                f"Symbol count {len(order_map)} != n_shells*n_species ({expected}).")
        return order_map
    ...
    def propagate(self, population, times, launch=None):
        ...
        if output.success:
            ...
            # Transpose to make it [time, variables]
            results_matrix = output.y.T
            return results_matrix
        else:
            print(f"Model run failed: {output.message}")
            return None
    ...
    def lambdify_launch_elliptical(self, full_lambda=None):
        ...
        for species_lambda in full_lambda:
            if species_lambda is None:
                if self.elliptical:
                    full_lambda_flattened.extend(
                        [None] * (self.n_shells * self.n_ecc_bins))
                else:
                    full_lambda_flattened.extend([None] * self.n_shells)
                continue
        ...
        for i, species_lambda in enumerate(full_lambda):
            if species_lambda is None:
                idx += self.n_shells * \
                    (self.n_ecc_bins if self.elliptical else 1)
                continue
        ...
        if self.elliptical:
            ...
            full_lambda_reshaped = np.transpose(
                reshaped, (1, 0, 2))  # → (sma, species, ecc)
        else:
            reshaped = np.array(full_lambda_flattened, dtype=object).reshape(
                (len(species_names), self.n_shells)
            )
            full_lambda_reshaped = np.transpose(
                reshaped, (1, 0))  # → (sma, species)
